...
class Zip
{
    ...
    public function make($pathToFile, $type = 'zip')
    {
        ...
        $objectOrName = $type;
        if (is_string($type)) {
            $objectOrName = 'App\Models\Zip\Repositories\\' . ucwords($type) . 'Repository';
        }
    ...
    {
        $newImageNames = $this->repository->resetImageNames($currentFolder, $imageNames);
    ...
    {
        $this->extractOneFileS3($fileName);
    }
     ...
     * @param $path
     */
    private function extractOneFileLocal($fileName, $path){
        ...
        $fileStream = $this->getRepository()->getFileStream(str_replace('/', '\\', $fileName));
